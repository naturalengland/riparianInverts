...
events_code <- read_csv("../data/events_code_lookup_aug_2019.csv")
...
biol_records <- biol_records %>% 
  mutate(Sample_Type_old = Sample_Type, 
         Sample_Type = recode_factor(tolower(biol_records$Sample_Type),
             ...
             `30 minute timed sample` = "hand_search",
             aquatic = "aquatic",
             `casual record` = "hand_search",
             excavation = "excavation",
             `field observation` = "hand_search",
...
```
```{r}
biol_records %>% select(Sample_Type, Sample_Type_old) %>% group_by(Sample_Type) %>% count(Sample_Type_old) %>% 
...
###Create and import event code lookup table: 

The following code does not need to be run again as the current version of the events lookup table `events_code` is loaded at the beginning of this script.  The code is retained for the record.  
```{r}
...
# #make more changes in excel: renamed events and event codes to correct errors and for consistence.  now on v.3
# 
# events_code <- read_csv("../data/events_code_lookup_aug_2019.csv")
...
biol_records <- biol_records %>% 
  left_join(
    select(events_code, event, event_code_v3, event_name_v3),
    by = "event") %>%
  mutate(event = as.factor(event)) %>%
  mutate(event_code = as.factor(event_code_v3)) %>%
  select(-event_code_v3) %>%
  mutate(event = as.factor(event_name_v3)) %>%
  select(-event_name_v3)
...
field_data <- field_data %>% 
  mutate(gridref = str_remove_all(gridref, " ")) %>%  #remove whitespace
  mutate(gridref = toupper(gridref)) #make letters uppercase

# calculate grid ref precision
field_data$coord_precision <- ifelse(grepl("^[a-zA-Z]{2}[0-9]{4}$",
                                              field_data$gridref),'1000','NA')

field_data$coord_precision[grepl("^[a-zA-Z]{2}[0-9]{2}[a-zA-Z]{1}$",
                                    field_data$gridref)] <- '2000'

field_data$coord_precision[grepl("^[a-zA-Z]{2}[0-9]{2}$",
                                    field_data$gridref)] <- '10000'

field_data$coord_precision[grepl("^[a-zA-Z]{2}$",
                                    field_data$gridref)] <- '100000'

field_data$coord_precision[grepl("^[a-zA-Z]{2}[0-9]{6}$",
                                    field_data$gridref)] <- '100'

field_data$coord_precision[grepl("^[a-zA-Z]{2}[0-9]{8}$",
                                    field_data$gridref)] <- '10'

field_data$coord_precision[grepl("^[a-zA-Z]{2}[0-9]{10}$",
                                    field_data$gridref)] <- '1'

field_data$coord_precision <- as.numeric(field_data$coord_precision)

count(field_data, coord_precision)

#field_data %>% filter(coord_precision == "NA") %>% count(gridref)

```

```{r}
#convert grid refs to eastings and northings

field_data$easting<-NA
field_data$northing<-NA

if (nrow(field_data) > 0) {
  for (i in 1:nrow(field_data)) { #1850:1870) {   
    field_data$easting[i] <- ifelse(is.na(field_data[i, "gridref"]), NA,
                 rnrfa::osg_parse(field_data[i, "gridref"])[[1]])
    field_data$northing[i] <- ifelse(is.na(field_data[i, "gridref"]), NA,
                 rnrfa::osg_parse(field_data[i, "gridref"])[[2]])
  }
}


field_data %>% select(easting, northing) %>% sample_n(20)

```


```{r}
field_data %>% filter(event_code == "EdenRead_2000") %>% count(sample_type)
field_data %>% group_by(event_code) %>% count(sample_type) %>% filter(sample_type == "ad-hoc") %>% print() %>% clipr::write_clip()
```
...
```{r}
event_code_selected <- c("Frome_2017_2", "Frome_2017_1", 
                         "Frome_2017_3", "Lugg_2014_1", "Lugg_2014_3", 
                         "Lugg_2014_4", "Wye_2014_6", "Wye_2014_7", 
                         "Wye_2014_4", "Wye_2014_3", "Wye_2014_5", 
                         "Lugg_2014_2", "Wye_2014_1", "Dove_2013", 
                         "Till_2013", "Beamish_2013", "Wooler_2013")
...
```{r}
  ...
  write_csv(field_data_selected, "../data/field_data_selected_Aug2019.csv")
  write_csv(field_data_extract, "../data/field_data_extract_Aug2019.csv")
```
