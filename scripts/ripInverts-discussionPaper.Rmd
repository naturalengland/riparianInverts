...
`Species and Event data pivot table March with summary.xlsx` sheet 2 `Riparian Pantheon List` contains information about species characteristics
```{r}
head(pantheon_data)
skimr::skim_to_wide(pantheon_data)
```
...
ggplot(graphdata, aes(x = evt_year, y = richness, group = as.factor(evt_year))) +
  geom_boxplot(varwidth = T, colour = "darkgrey") +
  geom_point(position = position_jitter(width = 0.08, height = 0.3), size = 0.9)+
  scale_x_continuous(breaks = seq(1997,2020, by = 1), limits = c(1997, 2018), 
                     labels = c("", "", "", "2000", "", "", "", "", 
                                "2005", "", "", "", "", "2010", "", "", "", "", 
                                "2015", "", "", "", "", "2020")) +
  scale_y_continuous(breaks = seq(0, 250, by = 25))+
...
chidata <- obs_all_freq_types %>% 
  ...
  mutate(is_wetland = as.character(is_wetland)) %>%
  mutate(is_wetland = recode(is_wetland, 
                              "n" = "not-wetland", "y" = "wetland")) %>% 
  group_by(is_wetland) %>% 
  count(sample_types) %>% 
...
ggplot(graphdata, aes(x = spp_name, y = n_samples, group = spp_name))+#, alpha = sample_type, fill = genus))+
  geom_col(position = position_dodge(width = 3, preserve = "single"), width = 3,
