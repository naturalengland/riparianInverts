...
```{r}
obs_all_freq_types %>% count(wetland_spp)
obs_all_freq_types %>% count(is_wetland)
```
*Note issue with table: i need to recode wetland species*
...
```{r}
ggplot(graphdata, aes(sample_types)) +
  geom_bar(aes(fill = sample_types, alpha = is_wetland), position = position_stack()) +
#  theme(legend.position = "none") +
...
*note: this graph has issues because I need to re-code what is and isn't wetland species.  Will be fixed, may affect results. *
